...
Функция ДанныеHDVINNConCustCountTender()
	...
	Попытка
		...
		ВыполнитьПакетЗапроса(мADOСоединение, ТекстЗапроса);
...
Процедура ВыполнитьПакетЗапроса(Соединение,ПакетЗапроса)
	
	Команда = Новый ComObject("ADODB.Command");
	Команда.CommandText = ПакетЗапроса;
...
Процедура ADOClose(Conection)
	
	Если Conection <> Неопределено Тогда 
		Попытка
			Conection.Close();
		Исключение
		КонецПопытки;
